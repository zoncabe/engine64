...
{
    switch (state) {
        case WALKING:   return LOCOMOTION_WALK;
        case RUNNING:   return LOCOMOTION_RUN;
        case SPRINTING: return LOCOMOTION_SPRINT;
...
static void actorMotion_setHorizontalVelocity(Entity *entity, float yaw, float target_speed, float response_rate, float dt)
{
    ...
    float target_vx = target_speed *  sinf(rad(yaw));
    float target_vy = target_speed * -cosf(rad(yaw));

    float factor = expf(-response_rate * dt);
...
static void actorMotion_setRotation(Entity *entity)
{
    ...
    const float velocity_yaw = deg(atan2(-body->velocity.x, -body->velocity.y));
    ...
    if (fabsf(target_yaw - entity->transform.rotation.z) > ACTOR_ROTATION_SNAP_THRESHOLD)
        entity->transform.rotation.z = angle_wrap(lerpf(entity->transform.rotation.z, target_yaw, ACTOR_ROTATION_LERP_FACTOR * data->horizontal_speed / settings->locomotion[LOCOMOTION_SPRINT].target_speed));
    else
...
static void (*actorMotion_handler[ACTOR_STATE_COUNT])(Entity *, MotionCommand *, float) = {
    [STANDING_IDLE] = actorMotion_setLocomotion,
    [WALKING]       = actorMotion_setLocomotion,
    [RUNNING]       = actorMotion_setLocomotion,
    [SPRINTING]     = actorMotion_setLocomotion,
