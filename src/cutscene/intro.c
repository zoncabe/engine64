...
static const float intro_transitions[INTRO_SPRITE_COUNT + 1] = { 3.00f, 5.00f, 7.00f, 10.0f };
		...
		if (t >= start && t < end) {
			...
			if (fade_progress > 0.0f) {
				intro_screen.element[1] = (DrawElement){
					.type = DRAW_RECTANGLE,
					.rectangle = { .fill = SHAPE_FILL_SOLID, .position = { 0.0f, 0.0f }, .size = { 320.0f, 240.0f }, .color = RGBA32(0, 0, 0, (uint8_t)(fade_progress * 255)) }
				};
