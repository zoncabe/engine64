	...
	.section = {
		[0] = {
			.element = {
				...
				[MAIN_MENU_TITLE]       = { .type = DRAW_TEXT,      .position = {  43.0f,  55.0f },                              .text      = { HEADLINER_40, MENU_STYLE_NORMAL, "Demo 64", &h40_parms } },
...
static ScreenAnimationTrack main_menu_transition_track[] = {
	...
	{ .target = &main_menu_screen.section[0].element[MAIN_MENU_TITLE].position.x,   .from = -260.0f, .to =  43.0f, .delay = 0.00f, .duration = 0.25f, .easing = SCREEN_ANIMATION_EASING_CUBIC_OUT },
	...
	{ .target = &main_menu_screen.section[0].element[MAIN_MENU_CREDITS].position.x, .from = 400.0f, .to = 45.0f, .delay = 0.21f, .duration = 0.25f, .easing = SCREEN_ANIMATION_EASING_CUBIC_OUT },
